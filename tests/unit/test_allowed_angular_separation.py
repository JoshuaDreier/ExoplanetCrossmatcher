...
def test_known_pm_and_epoch_uses_computed_radius():
    # radius = (pm + pm_err) * |epoch - 2000| + minimum = (1.0 + 0.5)*10 + 10
...
def test_masked_pm_or_epoch_gets_unknown_default():
    ...
    radii = allowed_angular_separation(pm, pmerr, epoch, unknown_default=42 * u.arcsec)
...
def test_masked_pm_err_contributes_zero_not_unknown_default():
    ...
    radii = allowed_angular_separation(pm, pmerr, epoch, unknown_default=50 * u.arcsec)
