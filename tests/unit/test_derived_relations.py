...
def _nea_source(teff=5778.0, rad=None, mass=None, logg=None, insol=None, eqt=None, lum=None, spectype="G2V", teff_masked=False):
    ...
    nea._lookup = nea._build_lookup(Table({
        ...
        "st_lum":      [lum if lum is not None else np.nan],
        "st_lumerr1":  [0.05 * lum if lum is not None else np.nan],
        "st_lumerr2":  [-0.04 * lum if lum is not None else np.nan],
    }))
    return nea
...
def test_teff_derived_from_spectype():
    ...
    # The G class range is [5440, 5930].
    # So upper error (err1) = 5930 - 5780 = 150 K, lower error (err2) = 5780 - 5440 = 340 K (positive magnitude)
    ...
    assert float(result["st_teff_err2"][0]) == pytest.approx(340.0, rel=1e-4)
...
def test_teff_derived_from_rad_and_lum():
    ...
    # T = T_sun * (L / R^2)^0.25. Pass lum=0.0 (log_lum=0.0) -> linear lum=1.0
...
def test_rad_derived_from_lum_and_teff():
    ...
    # R = sqrt(L) * (T_sun / T)^2. Pass lum=0.0 (log_lum=0.0) -> linear lum=1.0
