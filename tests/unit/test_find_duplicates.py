...
def test_find_duplicates_not_full(input_table, cm_with_duplicates):
    result = cm_with_duplicates.find_duplicates(input_table, "star_name", full=False)
    assert len(result) == 1
...
def test_find_duplicates_full(input_table, cm_with_duplicates):
    result = cm_with_duplicates.find_duplicates(input_table, "star_name", full=True)
    assert len(result) == 2
    for row in result:
...
def test_find_duplicates_no_false_positives(cm_with_duplicates, input_table):
    result = cm_with_duplicates.find_duplicates(input_table, "star_name")
    for row in result:
