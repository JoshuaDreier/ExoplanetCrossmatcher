"""Unit tests for the ``<param>_key`` override mechanism in ``ParamFiller.enrich()[0]``.
...
def test_override_output_column_uses_custom_name():
    ...
    result = ParamFiller([nea]).enrich(cat, **{**DEFAULT_ENRICH_KEYS, "star_radius_key": "my_rad"})[0]
...
def test_override_absent_column_falls_back_to_nea():
    ...
    result = ParamFiller([nea]).enrich(cat, **{**DEFAULT_ENRICH_KEYS, "star_radius_key": "missing_col"})[0]
...
def test_multiple_overrides_independent():
    ...
    result = ParamFiller([nea]).enrich(cat, **{**DEFAULT_ENRICH_KEYS,
                                        ...
                                        "star_effective_temperature_key": "my_teff"})[0]
...
def test_planet_flux_key_overrides_pl_insol():
    ...
    result = ParamFiller([nea]).enrich(cat, **{**DEFAULT_ENRICH_KEYS, "planet_flux_key": "my_flux"})[0]
...
def test_star_effective_temperature_key_override():
    ...
    result = ParamFiller([nea]).enrich(cat, **{**DEFAULT_ENRICH_KEYS, "star_effective_temperature_key": "my_teff"})[0]
...
def test_unified_folding_of_all_key_params():
    ...
    result1 = ParamFiller([nea]).enrich(cat, **{**DEFAULT_ENRICH_KEYS, "star_radius_key": "my_radius"})[0]
