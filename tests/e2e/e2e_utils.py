...
def get_planets_for_star(star_name, ra, dec, crossmatcher, crossmatching_method):
    """Helper function to get planets for a star using the specified crossmatching method"""
    query_row = Table(
        rows=[(star_name, ra, dec)],
...
def _e2e_all_planets_found(star_name, ra, dec, expected_planets, crossmatcher, crossmatching_method):
    ...
    matched_planets = get_planets_for_star(star_name, ra, dec, crossmatcher, crossmatching_method)
...
def _e2e_no_false_positives(star_name, ra, dec, expected_planets, crossmatcher, crossmatching_method):
