...
class NeaParamSource(ParamSource):
    ...
    Provides: teff, rad, mass, logg, insol, dist, met, lum, pl_eqt, vmag, kmag, spec.
    """
    ...
    def _build_lookup(self, table: Table) -> dict:
        """Build lookup dict for NEA source using common helper.

        The NEA table uses ``pl_name`` as its identifier column, which matches the
