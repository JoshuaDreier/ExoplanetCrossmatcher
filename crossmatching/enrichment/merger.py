...
def _derive_stellar_teff(
    ...
    spec_src : str
        Source of stellar spectral type.
    ...
    if not rad.mask and not lum.mask and rad.val > 0 and lum.val > 0:
        ...
        # Error propagation
        t_l_up = _safe_err_sq(lum.err1, lum.val, 0.25)
        t_l_dn = _safe_err_sq(lum.err2, lum.val, 0.25)
        t_r_up = _safe_err_sq(rad.err1, rad.val, 0.5)
        t_r_dn = _safe_err_sq(rad.err2, rad.val, 0.5)
        
        err1 = t * np.sqrt(t_l_up + t_r_dn) if (t_l_up > 0 or t_r_dn > 0) else 0.0
        err2 = t * np.sqrt(t_l_dn + t_r_up) if (t_l_dn > 0 or t_r_up > 0) else 0.0
        return ParamQty(
            val=t,
            mask=False,
            src=f"SB_derived(rad:{rad.src} lum:{lum.src})",
            err1=err1,
            err2=err2
        )
    ...
    # 2. Try estimation from spectral type
        ...
        try:
            ...
            t = spectype_to_teff(spec_val)
            if t > 0:
                t_min, t_max = get_spectral_class_range(spec_val)
                ...
                return ParamQty(
                    val=t,
                    mask=False,
                    src=f"spectype_derived(spec:{spec_src})",
                    err1=err1,
                    err2=err2
                )
        except ValueError:
...
def _derive_stellar_radius(
    ...
    # 1.5. Try exact physical Stefan-Boltzmann relation next
        ...
        return ParamQty(
            val=r,
            mask=False,
            src=f"SB_derived(lum:{lum.src} teff:{teff.src})",
            err1=err1,
            err2=err2
        )
...
class ParamFiller:
    ...
    def _merge_values(
        ...
        for i, row in enumerate(table):
            ...
            if not disable_calculations:
                ...
                params_q['st_teff'][i] = _derive_stellar_teff(
                    teff[i], rad[i], lum[i], spec_val, spec_src
                )

                # Radius derivation: if radius not bound, attempt estimation using helpers
                params_q['st_rad'][i] = _derive_stellar_radius(
                    rad[i], mass[i], logg[i], teff[i], lum[i], met[i], kmag[i], dist[i], spec_val
                )

                # Mass derivation from logg and radius
                params_q['st_mass'][i] = _derive_stellar_mass(
                    mass[i], rad[i], logg[i]
                )
    ...
    def enrich(
        self,
        table: Table,
        planet_radius_key: str,
        planet_flux_key: str,
        planet_equilibrium_temperature_key: str,
        semi_major_axis_key: str,
        period_key: str,
        msini_key: str,
        star_spectral_type_key: str,
        star_radius_key: str,
        star_mass_key: str,
        star_effective_temperature_key: str,
        star_logg_key: str,
        star_metallicity_key: str,
        star_luminosity_key: str,
        vmag_key: str,
        kmag_key: str,
        distance_key: str,
